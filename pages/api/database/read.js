  ...
  if (connection) {
    if (body === 'playersMatches') {
      ...
      for (let i = 1; i < tableNumberRows; i += n) {
        dataPlayersMatches.push(
          ...queryMySql(`SELECT * FROM PLAYERS_MATCHES join PLAYERS on PLAYERS_MATCHES.account_id = PLAYERS.account_id LIMIT ${i},${n} ;`),
        );
      }
      ...
      for (let i = 1; i < tableNumberRows; i += n) {
        dataMatches.push(
          ...queryMySql(`SELECT match_id FROM MATCHES LIMIT ${i},${n};`)
      ...
      for (let i = 1; i < tableNumberRows; i += n) {
        dataPlayers.push(
          ...queryMySql(`SELECT account_id FROM PLAYERS LIMIT ${i},${n};`)
